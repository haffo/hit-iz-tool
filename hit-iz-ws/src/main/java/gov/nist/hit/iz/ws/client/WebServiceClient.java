...
public class WebServiceClient implements TransportClient {
	...
	@Override
	public String send(String soapEnvelope, Object... arguments)
			throws TransportClientException {
		String res = null;
		try {
			final String endpoint = (String) arguments[0];
			StreamSource source = new StreamSource(new StringReader(
					soapEnvelope));
			StringWriter stringWriter = new StringWriter();
			StreamResult xmlOutput = new StreamResult(stringWriter);

			webServiceTemplate.sendSourceAndReceiveToResult(endpoint, source,
					xmlOutput);
	...
	private String toString(SoapFaultClientException e)
			throws TransportClientException {
		try {
			StringWriter stringWriter = new StringWriter();
			StreamResult xmlOutput = new StreamResult(stringWriter);
