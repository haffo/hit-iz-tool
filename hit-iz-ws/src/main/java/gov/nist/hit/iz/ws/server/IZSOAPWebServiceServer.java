...
public class IZSOAPWebServiceServer implements TransportServer {
	...
	private void validateRequest(SubmitSingleMessageRequestType request)
		...
		// else if (!TransactionStatus.OPEN.equals(getStatus(username, password,
		// facilityID))) {
		// throw new TransportServerException("Transaction not initialized
		// correctly");
		// }
	}

	// private SubmitSingleMessageResponseType
	// getPreCannedSubmitSingleMessageResponse() {
	...
	// "Sorry, Server Error. Failed to generate response message
