...
public class TestCaseController extends TestingController {
  ...
  static final Logger logger = LoggerFactory.getLogger(TestCaseController.class);
  ...
  public void testPackage(@PathVariable Long testCaseId, HttpServletRequest request,
    ...
    try {
      ...
      if (testPackagePath == null) {
        throw new IllegalArgumentException("No test package found for this TestCase");
      }
      InputStream content = TestCaseController.class.getResourceAsStream(testPackagePath);
      response
  ...
  @RequestMapping(value = "/teststory/download", method = RequestMethod.POST,
  ...
  public String testProcedure(@PathVariable Long testPlanId, HttpServletRequest request,
    ...
    try {
      ...
      if (testProcedurePath == null) {
        throw new IllegalArgumentException("No test procedure found for this TestPlan");
      }
      InputStream content = TestCaseController.class.getResourceAsStream(testProcedurePath);
      response
          .setContentType("application/vnd.openxmlformats-officedocument.wordprocessingml.document");
