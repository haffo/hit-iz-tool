...
public class ConnectivityController extends TestingController {
	...
	static final Logger logger = LoggerFactory
			.getLogger(ConnectivityController.class);
	...
	public SoapConnectivityTestCase testCase(@PathVariable final Long testCaseId) {
		SoapConnectivityTestCase testCase = testCaseRepository
				.findOne(testCaseId);
		if (testCase == null)
		...
		try {
			...
			if (testCase == null)
				throw new TestCaseException(
						"No testcase found. Invalid testCase id=" + testCaseId);
			SoapConnectivityTestContext context = testCase.getTestContext();
			if ("req".equals(type)) {
				if (testCase.getTestType().equals(
						IZTestType.RECEIVER_UNSUPPORTED_OPERATION.toString())
						|| testCase.getTestType().equals(
								IZTestType.SENDER_UNSUPPORTED_OPERATION
										.toString())) {
				...
				return soapValidator.validate(Utils.getContent(command),
						testCase.getName(),
						context.getResponseValidationPhase(),
	...
	public Command sendRequest(@RequestBody SoapConnectivityCommand command)
		...
		try {
			...
			if (!IZTestType.RECEIVER_CONNECTIVITY.toString().equals(
					testCase.getTestType())) {
				request = ConnectivityUtil.updateSubmitSingleMessageRequest(
						req, command.getU(), command.getP(),
						command.getFacilityId());
			} else if (IZTestType.RECEIVER_CONNECTIVITY.toString().equals(
					testCase.getTestType())) {
				request = ConnectivityUtil.updateConnectivityRequest(req);
			}
			String response = transportClient.send(request,
					command.getEndpoint());
			String tmp = response;
			try {
	...
	public boolean initIncoming(@RequestBody final User user)
			throws UserTokenIdNotFoundException {
		logger.info("Initializing transaction for username ... "
				+ user.getUsername());
	...
	public SoapConnectivityTransaction transaction(@RequestBody final User user) {
		...
		SoapConnectivityTransaction transaction = transactionRepository
				.findByUsernameAndPasswordAndFacilityID(user.getUsername(),
						user.getPassword(), user.getFacilityID());
		return transaction != null ? transaction
				: new SoapConnectivityTransaction();
	}
