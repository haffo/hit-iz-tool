 ...
 */
public class Er7ValidationResult extends gov.nist.hit.core.domain.ValidationResult {
  ...
  public Er7ValidationResult(MessageValidationResultV2 result, String title) {
    if (result != null) {
      ...
      Iterator<gov.nist.healthcare.core.validation.message.v2.MessageFailureV2> it =
          result.getErrors();
      while (it.hasNext()) {
  ...
  private String generateXml(String title, HL7V2MessageValidationReportDocument report) {
    ...
    tc.setName(title);
    ReportHeader header = report.getHL7V2MessageValidationReport().getHeaderReport();
