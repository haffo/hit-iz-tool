...
public class CDCSoapValidation {
	...
	private ArrayList<MessageFailureV3> verifySubmitSingleMessageRequest(
			String soapMessage, String phase) {
		ArrayList<MessageFailureV3> soapFailures = new ArrayList<MessageFailureV3>();
		try {
			Node content = ConnectivityUtil.findNode(soapMessage,
			...
			if (content != null) {
				...
				if (hl7Message.startsWith("<![CDATA[")) {
					...
					hl7Message = hl7Message.substring(0,
							hl7Message.lastIndexOf(cdataEnd));
				}
				try {
	...
	private ArrayList<MessageFailureV3> verifyConnectivityResponse(
			String soapMessage, String requestMessage) {
		ArrayList<MessageFailureV3> soapFailures = new ArrayList<MessageFailureV3>();
		try {
			if (requestMessage != null) {
				Node eckoBackNode = ConnectivityUtil
						.findNode(
								requestMessage,
								"//*[local-name()='Envelope']/*[local-name()='Body']/*[local-name()='connectivityTest']/*[local-name()='echoBack']");
				String echoBack = eckoBackNode != null ? eckoBackNode
						.getTextContent() : null;
				Node returnNode = ConnectivityUtil
						.findNode(
								soapMessage,
								"//*[local-name()='Envelope']/*[local-name()='Body']/*[local-name()='connectivityTestResponse']/*[local-name()='return']");
				String returnedValue = returnNode != null ? returnNode
						.getTextContent() : null;
				if (returnNode == null || returnedValue == null) {
					...
					mf.setPath(path);
					soapFailures.add(mf);
				} else if (echoBack != null
						&& !returnedValue.contains(echoBack)) {
					MessageFailureV3 mf = new MessageFailureV3();
					mf.setFailureType(AssertionTypeV3Constants.SOAP);
					mf.setFailureSeverity(ErrorSeverityConstants.NORMAL);
					mf.setAssertionResult(AssertionResultConstants.ERROR);
					mf.setDescription("The returned value '" + returnedValue
							+ "' does not contain '" + echoBack + "'");
	...
	public gov.nist.healthcare.core.validation.soap.SoapValidationResult validate(
			SoapMessage soapMessage, InputStream schematron, String... options)
			throws XmlException {
	...
	private void setErrLineNumbers(String soapXml,
		...
		while (iterator.hasNext()) {
			...
			if (targetPath == null) {
				System.out.println("No location available for #"
						+ soapFailures.indexOf(failure));
				continue;
			}
