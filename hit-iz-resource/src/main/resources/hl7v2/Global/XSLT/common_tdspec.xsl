<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:util="http://hl7.nist.gov/data-specs/util" xpath-default-namespace="" exclude-result-prefixes="xs" version="2.0">
	...
	<xsl:variable name="version" select="'2.5'"/>
	...
	<xsl:template name="_main">
		<xsl:variable name="message-type">
			<xsl:choose>
				<xsl:when test="starts-with(name(.), 'SS')">
	...
	<xsl:template match="PID" mode="Syndromic">
		...
		<xsl:param name="counter"/>
			<xsl:value-of select="util:title('title', concat('Patient Information', $counter), 'Patient Information', $ind1, false(), $vertical-orientation)"/>
			<xsl:value-of select="util:elements($ind1)"/>
			<xsl:value-of select="util:element('Name', util:valueset(PID.5.7, 'HL70200'), $ind1)"/> <!--                      ????????????????????????????                                  PID.5[2].7 ????? -->
			...
			<xsl:value-of select="util:element('State', .//PID.11.4, $ind1)"/>
			<xsl:value-of select="util:element('Zip Code', .//PID.11.5, $ind1)"/>
			<xsl:value-of select="util:element('Country', .//PID.11.6, $ind1)"/>
			...
			<xsl:variable name="yes" as="xs:boolean" select=".//PID.7.1 = 'Y' or .//PID.7.1 = 'y'"/>
			<xsl:value-of select="util:last-element('Patient Death Indicator', util:IfThenElse($yes, 'Yes', //PID.30), $ind1, $vertical-orientation)"/>  <!--- MSH.9.2 == A01 do not include this; and the one above -->				
	</xsl:template>
	...
	<xsl:template match="PV1" mode="Syndromic">
		...
		<xsl:param name="counter"/>
			...
			<xsl:value-of select="util:elements($ind1)"/>
			<xsl:value-of select="util:element('Admit or Encounter Reason', util:value-or-valueset(..//PID.10.2, ..//PID.10.1, 'CDCREC'), $ind1)"/> 
			<xsl:value-of select="util:element('Admit Date and Time', util:format-date(.//PV1.44), $ind1)"/> <!-- - - - - - time? - - - - - - - - - - - -->
			<xsl:value-of select="util:element('Patient Class', util:valueset(PV1.2, 'PatientClass'), $ind1)"/>
			...
			<xsl:value-of select="util:last-element('Diagnosis', util:value-or-valueset(..//DG1.3.2, ..//DG1.3.3, 'CDCREC'), $ind1, $vertical-orientation)"/>  <!-- ????? Discease CDREC??? -->
	</xsl:template>
	...
	<xsl:template xmlns:fo="http://www.w3.org/1999/XSL/Format" name="css">
		<style xmlns="" type="text/css">
			...
			.test-data-specs-main .panel-heading { font: bold; background-color: #D9EDF7 !important; }					
			.test-data-specs-main .accordion-heading { font-weight:bold; font-size:90%;  color: #003399;}										
			}
